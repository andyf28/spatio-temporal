...
class TaxiDataVisualizer:
    ...
    def prepare_data(self):
        self.df['pickup_datetime'] = pd.to_datetime(self.df['tpep_pickup_datetime']) # renames df column
        # extraction of time values from time column
    ...
    # temporal month heatmap (uncomment to plot)
...
output_path_year = f"~/Desktop/spatio-temporal/plots/{NAME_YEAR}"
output_path_month = f"~/Desktop/spatio-temporal/plots/{NAME_MONTH}"

visualizer = TaxiDataVisualizer(parquet_path)
visualizer.plot_month_heatmap(output_path_month) # change both plot_xxx_heatmap or output_path_xxx to either year or month
