...
class TaxiDemandForecast:
    ...
    def load_data(self):
        ...
        self.df = self.df[ 
            (self.df['tpep_dropoff_datetime'] >= '2024-01-01 00:00:00') & 
            (self.df['tpep_dropoff_datetime'] <= '2024-11-30 23:59:59') 
        ]
        self.df = self.df.sort_values(by=['tpep_dropoff_datetime', 'DOLocationID'])
        ...
        print("Data date range:", self.df['tpep_dropoff_datetime'].min(), "to", self.df['tpep_dropoff_datetime'].max())
    ...
    def aggregate_data(self):
        ...
        with tqdm(total=1, desc="Aggregating trips") as pbar:
            self.df = self.df.groupby([self.df['tpep_dropoff_datetime'].dt.date, 'DOLocationID'])\
    ...
    def create_features(self):
        ...
        with tqdm(total=len(feature_steps), desc="Creating features") as pbar:
            self.df['dayofweek'] = self.df['tpep_dropoff_datetime'].dt.dayofweek
            self.df['month'] = self.df['tpep_dropoff_datetime'].dt.month
            self.df['is_weekend'] = (self.df['dayofweek'] >= 5).astype(int)
            pbar.update(1)

            # Calculate mean trip count per location to use for imputation
            location_means = self.df.groupby('DOLocationID')['trip_count'].mean().to_dict()
            
            self.df['trip_count_lag1'] = self.df.groupby('DOLocationID')['trip_count'].shift(1)
            self.df['trip_count_lag7'] = self.df.groupby('DOLocationID')['trip_count'].shift(7)
            # Impute missing lag values with the location's mean
            self.df['trip_count_lag1'] = self.df.apply(
                lambda row: location_means.get(row['DOLocationID'], 0) if pd.isna(row['trip_count_lag1']) else row['trip_count_lag1'], axis=1)
            self.df['trip_count_lag7'] = self.df.apply(
                lambda row: location_means.get(row['DOLocationID'], 0) if pd.isna(row['trip_count_lag7']) else row['trip_count_lag7'], axis=1)
            pbar.update(1)

            self.df['rolling_avg'] = self.df.groupby('DOLocationID')['trip_count']\
            ...
            self.df['rolling_avg'] = self.df.apply(
                lambda row: location_means.get(row['DOLocationID'], 0) if pd.isna(row['rolling_avg']) else row['rolling_avg'], axis=1)
            pbar.update(1)
    ...
    def split_data(self):
        ...
        val = pd.get_dummies(val, columns=['DOLocationID'], prefix='loc')
        test = pd.get_dummies(test, columns=['DOLocationID'], prefix='loc')
        ...
        feature_cols = [col for col in all_columns if col not in ['trip_count', 'tpep_dropoff_datetime']]
    ...
    def visualise_predictions(self):
        """Creates a heatmap of predicted average daily taxi dropoffs for 2025 and saves to CSV"""
        print("Generating predictions for 2025...")
        
        locations = [loc for loc in self.df['DOLocationID'].unique() if loc in self.allowed_location_ids]
        ...
        historical_means = self.df.groupby('DOLocationID')['trip_count'].mean().to_dict() # calculates historical means for each location
        
        for loc in tqdm(locations, desc="Predicting for locations"):
            df_loc = self.df[self.df['DOLocationID'] == loc].copy()
            df_loc = df_loc.sort_values('tpep_dropoff_datetime')
            ...
            predictions.append({'DOLocationID': loc, 'pred_trip_count': mean_pred_daily})
        ...
        heatmap_data = self.nyc_zones.merge(pred_df, left_on='location_id', right_on='DOLocationID')
